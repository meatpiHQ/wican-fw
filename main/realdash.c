...
uint8_t real_dash_set_66(twai_message_t *msg, uint8_t *buf)
{
	...
	buf[frame_len++] = 0x33;
	buf[frame_len++] = 0x22;
	buf[frame_len++] = 0x11;		//8bytes
	buf[frame_len++] = (msg->identifier & 0xFF);
	buf[frame_len++] = (msg->identifier & 0x0000FF00)>>8;
	buf[frame_len++] = (msg->identifier & 0x00FF0000)>>16;
	buf[frame_len++] = (msg->identifier & 0xFF000000)>>24;

	for(i = 0; i < msg->data_length_code; i++)
	{
		buf[frame_len++] = msg->data[i];
	}
	if(msg->data_length_code < 8)
	{
		for(i = frame_len; i < 16; i++)
		{
			buf[frame_len++] = 0x00;
		}
	}

	crc = Crc32(buf, frame_len);
	buf[frame_len++] = (crc & 0xFF);
	buf[frame_len++] = (crc & 0x0000FF00)>>8;
	buf[frame_len++] = (crc & 0x00FF0000)>>16;
	buf[frame_len++] = (crc & 0xFF000000)>>24;
...
uint8_t real_dash_parse_66(twai_message_t *msg, uint8_t *buf)
{
	...
	if((buf[0] == 0x66) && (buf[1] == 0x33) && (buf[2] == 0x22))
	{
		if(buf[3] == 0x11)//only 8 bytes
		{
			crc = Crc32(buf, 16);
			if( (buf[16] == (crc & 0xFF)) && (buf[17] == ((crc & 0x0000FF00)>>8)) && (buf[18] == ((crc & 0x00FF0000)>>16)) && (buf[19] == ((crc & 0xFF000000)>>24)) )
			{
