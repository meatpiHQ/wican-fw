...
const char device_config_default[] = "{\"wifi_mode\":\"AP\",\"ap_ch\":\"6\",\"webhook_en\":\"enable\",\"sta_ssid\":\"MeatPi\",\"sta_pass\":\"TomatoSauce\",\"sta_security\":\"wpa3\",\
								...
								\"batt_mqtt_pass\":\"meatpi\",\"batt_alert_time\":\"1\",\"mqtt_en\":\"disable\",\"mqtt_elm327_log\":\"disable\",\"elm327_udp_log\":\"disable\",\"mqtt_url\":\"mqtt://127.0.0.1\",\"mqtt_port\":\"1883\",\
										...
										\"logger_status\":\"disable\",\"csv_log\":\"disable\",\"log_filesystem\":\"littlefs\",\"log_storage\":\"sdcard\",\"log_period\":\"10\",\"csv_format\":\"wide\",\"csv_grid_mode\":\"fixed\",\"csv_grid_hz\":\"10\"}";
...
//	cJSON_AddStringToObject(root, "can_datarate", device_config.can_datarate);
	...
	cJSON_AddStringToObject(root, "csv_format", device_config.csv_format);
...
static void config_server_load_cfg(char *cfg)
{
	...
	//***** Wide CSV (Task #11): csv_format / csv_grid_mode / csv_grid_hz. Garbage coerces
	//      to a safe default so a bad NVS value can never select an invalid layout.
	key = cJSON_GetObjectItem(root,"csv_format");
	if(key == 0)
	{
		strlcpy(device_config.csv_format, "wide", sizeof(device_config.csv_format));
	}
	else
	{
		strlcpy(device_config.csv_format, key->valuestring, sizeof(device_config.csv_format));
	}
	if(strcmp(device_config.csv_format, "long") != 0 && strcmp(device_config.csv_format, "wide") != 0)
	{
		strlcpy(device_config.csv_format, "wide", sizeof(device_config.csv_format));
	}
	ESP_LOGI(TAG, "device_config.csv_format: %s", device_config.csv_format);
	//*****
...
{
	if(strcmp(device_config.csv_format, "wide") == 0)
	{
		return 1;
	}
	else if(strcmp(device_config.csv_format, "long") == 0)
	{
		return 0;
	}
	return -1;
