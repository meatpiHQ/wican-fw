...
static void config_server_load_cfg(char *cfg)
{
	...
	if(key == 0 || (strlen(key->valuestring) > sizeof(device_config.mqtt_rx_topic)))
	{
		goto config_error;
	}
	if(strlen(key->valuestring) == 0)
	{
			sprintf(device_config.mqtt_rx_topic, "wican/%s/can/rx", device_id);
	}
	else
	{
