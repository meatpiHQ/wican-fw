...
//        httpd_resp_send_chunk(req, buf, ret);
	...
	FILE* f = fopen("/spiffs/mqtt_canflt.json", "w");
	if (f != NULL)
	{
		fputs(buf, f);
		fclose(f);
...
//			fwrite(device_config_default , 1 , sizeof(device_config_default) , f );
		...
		FILE* f1 = fopen("/spiffs/mqtt_canflt.json", "r");
		if (f1 != NULL)
		{
			fseek(f1, 0, SEEK_END);
			filesize = ftell(f1);
			...
			fseek(f1, 0, SEEK_SET);
			ESP_LOGI(TAG, "mqtt_canflt.json: %s", mqtt_canflt_file);
		}
