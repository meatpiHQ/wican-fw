...
static char *auto_pid_file = NULL;
...
{
	if(auto_pid_file != NULL)
	{
		const char* resp_str = (const char*)auto_pid_file;
		httpd_resp_send(req, (const char*)resp_str, HTTPD_RESP_USE_STRLEN);
		ESP_LOGI(TAG, "auto_pid_file: %s", auto_pid_file);
	}
	else
	{
		const char* resp_str = (const char*) "NONE";
		httpd_resp_send(req, (const char*)resp_str, HTTPD_RESP_USE_STRLEN);
		ESP_LOGI(TAG, "auto_pid_file: NONE");
	}
...
static esp_err_t store_auto_data_handler(httpd_req_t *req)
{
	...
	// config_server_get_auto_pid();
	FILE* f1 = fopen("/fatfs/auto_pid.json", "r");
	if (f1 != NULL)
	{
		fseek(f1, 0, SEEK_END);
		int32_t filesize = ftell(f1);
		fseek(f1, 0, SEEK_SET);
		auto_pid_file = malloc(filesize+1);
		ESP_LOGI(__func__, "auto_pid File size: %ld", filesize);
		fseek(f1, 0, SEEK_SET);
		fread(auto_pid_file, sizeof(char), filesize, f1);
		auto_pid_file[filesize] = 0;
		fseek(f1, 0, SEEK_SET);
		ESP_LOGI(TAG, "auto_pid.json: %s", auto_pid_file);
	}
...
//            sizeof(server_data->base_path));
	...
	if(esp_fatfs_flag == 0) 
	{
		...
		if (f != NULL)
		{
			...
			if (auto_pid_file != NULL)
			{
				fread(auto_pid_file, sizeof(char), filesize, f);
				auto_pid_file[filesize] = 0;
				ESP_LOGI(TAG, "auto_pid.json: %s", auto_pid_file);
			}
			fclose(f);
