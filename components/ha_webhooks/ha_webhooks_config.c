...
esp_err_t ha_webhooks_init(void)
{
    ...
    if (!s_cfg_psram)
    {
        // Prefer PSRAM when present, but ESP32-C3 (and other targets) may not have it.
        ...
        if (!s_cfg_psram)
        {
            ...
            s_cfg_psram = (ha_webhook_config_t *)heap_caps_malloc(sizeof(ha_webhook_config_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        }
        if (!s_cfg_psram)
        {
            // Final fallback.
            s_cfg_psram = (ha_webhook_config_t *)malloc(sizeof(ha_webhook_config_t));
        }
        if (!s_cfg_psram)
        {
            ESP_LOGE(TAG, "Failed to allocate config cache");
            return ESP_ERR_NO_MEM;
        }
    ...
    // Missing config file is expected on fresh devices; keep defaults but report init OK.
